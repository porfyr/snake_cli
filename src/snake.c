...
static void choose_init_direction(Snake snake, int *direction_r, int *direction_c) {
    // here would be nice to generate direction oposite
    //  to the nearest wall but i'm lazy
    *direction_r = 0;
    *direction_c = -1;
}
...
int snake_add_part(Snake *p_snake, int direction_r, int direction_c)
{
    ...
    if (p_snake->sp_head == NULL) {
        sp.coords[0] = p_snake->p_map->height / 2;  // (rand() % p_snake->p_map->height-2) + 2;
        sp.coords[1] = p_snake->p_map->width  / 2;  // (rand() % p_snake->p_map->width-2) + 2;
...
static void snake_sync_map(const Snake *p_snake)
{
    ...
    while (current_sp != NULL) {
        ...
        // If this part is last and no matter if first (cleaning after tail)
        if (current_sp->next == NULL) {
